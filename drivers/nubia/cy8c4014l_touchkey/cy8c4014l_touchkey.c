...
static int cypress_set_mode(int mode) {
	...
	if(mode == CYPRESS4000_SLEEP_MODE) {
		if(g_cypress_touch_key_info->bLeftDown) {
			input_report_key(g_cypress_touch_key_info->platform_data->input_dev, KEY_F7, 0);
			input_sync(g_cypress_touch_key_info->platform_data->input_dev);
			g_cypress_touch_key_info->bLeftDown = false;
			pr_err("[%s] Send left up\n", __func__);
		}
		if(g_cypress_touch_key_info->bRightDown) {
			input_report_key(g_cypress_touch_key_info->platform_data->input_dev, KEY_F8, 0);
			input_sync(g_cypress_touch_key_info->platform_data->input_dev);
			g_cypress_touch_key_info->bRightDown = false;
...
static irqreturn_t cypress_touchkey_interrupt(int irq, void *dev_id)
{
	...
	if(val == 0xff && !g_cypress_touch_key_info->bLeftDown && !g_cypress_touch_key_info->bRightDown) {
    ...
    if(status & CYPRESS_LEFT_BIT)
    {
        ...
        input_sync(input);
		g_cypress_touch_key_info->bLeftDown = (cur_value & CYPRESS_LEFT_BIT)?true:false;
    }
    if(status & CYPRESS_RIGHT_BIT)
    {
        input_report_key(input, KEY_F8, (cur_value & CYPRESS_RIGHT_BIT)?1:0);
        input_sync(input);
		g_cypress_touch_key_info->bRightDown = (cur_value & CYPRESS_RIGHT_BIT)?true:false;
    }
	printk(">>>>>>>>>>>>cypress val = %d, cur_value = %d, status = %d keyst=[%d,%d]\n", val, cur_value, status, (int)g_cypress_touch_key_info->bLeftDown, (int)g_cypress_touch_key_info->bRightDown);
...
//    int fw_ver;
	...
	info->new_mode = CYPRESS4000_WAKEUP_MODE;
	info->bUpdateOver = false;
	info->bLeftDown = false;
	info->bRightDown = false;
