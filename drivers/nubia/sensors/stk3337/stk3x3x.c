...
static int stk3x3x_set_als_ctrl(struct stk3x3x_data *stk_data, uint8_t als_it, uint8_t als_gain);
...
static int32_t stk3x3x_als_calibration(struct stk3x3x_data *stk_data, const int source_cal_data)
{
	...
	/*als cail standard is STK_IT_ALS_100MS, STK_ALS_GAIN_4*/
	stk3x3x_set_als_ctrl(stk_data, STK_IT_ALS_100MS, STK_ALS_GAIN_4);
	...
	SENSOR_LOG_INFO("cali factor = %d\n", stk_data->lux_factor);
	ret = sensor_write_file(ALS_CAL_FILE_PATH, (char *)(&stk_data->lux_factor), sizeof(stk_data->lux_factor));
	if (ret >= 0)
		stk_data->lux_cailbrate = true;
...
uint32_t stk_alscode2lux(struct stk3x3x_data *stk_data, uint32_t alscode)
{
#ifdef STK_ALSPS_CALIBRATION
	uint32_t lux;
	lux = (alscode * stk_data->lux_factor) / 1000;
	return lux;
#else
...
static void stk3x3x_dump_reg(struct stk3x3x_data *stk_data)
{
	...
	for(cnt = 0; cnt < 13; cnt++) {
		...
		SENSOR_LOG_VERBOSE("reg[0x%2X]=0x%2X\n", reg[cnt], ret);
	}
}
...
static void stk_als_report(struct stk3x3x_data *stk_data, int als)
{
	...
	if(stk_data->als_dim_gain == false) {
		stk_data->als_lux_last = als;
		input_report_rel(stk_data->als_input_dev, REL_X, als+1);
	} else {
		stk_data->als_lux_last = als/16;
		input_report_rel(stk_data->als_input_dev, REL_Y, als+1);
	}
#ifdef QUALCOMM_PLATFORM
	...
	SENSOR_LOG_VERBOSE("als input event %d lux\n", stk_data->als_lux_last);
}
...
static int stk_prox_increment_init(struct stk3x3x_data *stk_data)
{
	...
	if(stk_data->cali_near_increment > PS_THRESH_INCREMENT_MAX || stk_data->cali_near_increment <= PS_FAR_DIFFER_VALUE)  {
	...
	SENSOR_LOG_ERROR("old high = %d, delta ht=%d, lt=%d\n",
	                 high_thd, stk_data->stk_ht_n_ct, stk_data->stk_lt_n_ct);

	SENSOR_LOG_ERROR("old smudge delta ht = %d, lt=%d\n",
	...
	SENSOR_LOG_ERROR("thd(%d, %d), smudge thd(%d, %d)\n",
...
static int32_t stk3x3x_get_als_reading(struct stk3x3x_data *stk_data)
{
	int32_t als_data, r_data, g_data, b_data, c_data, ir_data = 0;
#ifdef STK_ALS_FIR
	int index;
	...
	if(g_data > 0){
		...
		stk_data->als_cg_ratio = cg_ratio;
	}
	if(cg_ratio >=3 && cg_ratio < 9)
		als_data = (als_data*26)/(cg_ratio*10) - als_data/20;
	else if(cg_ratio >= 9){
		/*if cg_ratio > 9,the light source may be is D65, need adjust als_data*/
		als_data = (als_data*10)/LIGHT_SLOPE_D65;
	}
	...
	SENSOR_LOG_VERBOSE("ALS:als=%d, c=%d\n", als_data, c_data);
	SENSOR_LOG_VERBOSE("ALS:r=%d, g=%d, b=%d, cg_ratio=%d\n", r_data, g_data, b_data, cg_ratio);
...
static ssize_t stk_als_flush_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int32_t count,ret,als_reading,als_lux;
	int32_t retry_times = 10;
	...
	do {
		...
		ret = stk3x3x_get_flag(stk_data);
		retry_times--;
	} while(retry_times && !(ret&STK_FLG_ALSDR_MASK));
	if(retry_times <= 0) {
	...
	als_lux = stk_alscode2lux(stk_data, als_reading);
	stk_als_report(stk_data, als_lux);
	return count;
}
...
{
	struct stk3x3x_data *stk_data =  dev_get_drvdata(dev);
	if(stk_data->lux_cailbrate == false) {
		SENSOR_LOG_ERROR("stk als not calibrate");
		return 0;
	}
...
static ssize_t stk_code_debug_level_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", SENSOR_LOG_LEVEL);
}

static ssize_t stk_code_debug_level_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t size)
{
	int value = 0;
	int ret;
	...
	if(value <= 0)
		SENSOR_LOG_LEVEL = SENSOR_ERR;
	else
		SENSOR_LOG_LEVEL = value;
	SENSOR_LOG_ERROR("log level set %d", SENSOR_LOG_LEVEL);
	return size;
}
...
static ssize_t stk_als_firlen_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	SENSOR_LOG_INFO("len = %2d, idx = %2d\n", len, stk_data->fir.idx);
	SENSOR_LOG_INFO("sum = %5d, ave = %5d\n", stk_data->fir.sum, stk_data->fir.sum/len);
	return scnprintf(buf, PAGE_SIZE, "%d\n", len);
}
...
static ssize_t stk_ps_distance_flush_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	SENSOR_LOG_INFO(" ps flush input event=%d\n", dist);
	return scnprintf(buf, PAGE_SIZE, "%d\n", dist);
}
...
static int stk_ps_val(struct stk3x3x_data *stk_data)
{
	...
	if((stk_data->data_count%20)==5) {
		SENSOR_LOG_VERBOSE("bgir:%d, %d, %d, %d\n", value[0], value[1], value[2], value[3]);
		SENSOR_LOG_VERBOSE("psoff:%d, %d, %d, %d\n", psoff[0], psoff[1], psoff[2], psoff[3]);
	}
...
static int stk_ps_tune_zero_func_fae(struct stk3x3x_data *stk_data)
{
			...
			if (ret == 0) {
				...
				if (word_data > 0) {
					...
					if ((word_data < ct_value) && ((ct_value - word_data) > 10)
						...
						SENSOR_LOG_INFO("CTTRACKING pre ps=%d, count=%d\n", word_data, stk_data->tracking_count);
						if(stk_data->tracking_count > 3) {
							...
							if ((stk_data->ps_thd_h + STK_H_LT - STK_HT_N_CT) < stk_data->ps_high_thd_boot) {
								...
								stk_data->boot_cali = 1;
								SENSOR_LOG_INFO("update boot2 HT=%d, LT=%d\n",
								                stk_data->ps_high_thd_boot, stk_data->ps_low_thd_boot);
							}
						}
					} else {
	...
	if(diff > stk_data->stk_max_min_diff) {
		...
		if ((stk_data->ps_thd_h + STK_H_HT - STK_HT_N_CT) < stk_data->ps_high_thd_boot) {
			...
			stk_data->boot_cali = 1;
			SENSOR_LOG_INFO("update boot1 HT=%d, LT=%d\n",
			                stk_data->ps_high_thd_boot, stk_data->ps_low_thd_boot);
		}
...
static void stk_als_poll_work_func(struct work_struct *work)
{
	...
	/*dim light < 10Lux, amplify light value to correct the value is small*/
	if(stk_data->alsctrl_reg == (STK_IT_ALS_100MS|STK_ALS_GAIN_64) && reading_lux < 160)
		reading_lux = (208 - reading_lux)*reading_lux/48;
	...
	/*when fast report als equal false,we enable dim light gain(<200Lux)*/
	if(stk_data->als_fast_report == false) {
		if(stk_data->alsctrl_reg == (STK_IT_ALS_100MS|STK_ALS_GAIN_64)
			&& reading_lux > 3200
			&& (reading_lux - 3200)>160) {
			SENSOR_LOG_INFO("Nomal light, disable gain");
			stk3x3x_set_als_ctrl(stk_data, STK_IT_ALS_100MS, STK_ALS_GAIN_4);
			msleep(100);
			reading = stk3x3x_get_als_reading(stk_data);
#ifdef STK_ALS_FIR
			memset(&stk_data->fir, 0x00, sizeof(stk_data->fir));
#endif
		} else if(stk_data->alsctrl_reg == (STK_IT_ALS_100MS|STK_ALS_GAIN_4)
			&& stk_data->als_cg_ratio == 0
			&& reading_lux < 200
			&& (200 - reading_lux)>10) {
			SENSOR_LOG_INFO("Dim light,gain x16");
			stk3x3x_set_als_ctrl(stk_data, STK_IT_ALS_100MS, STK_ALS_GAIN_64);
			msleep(100);
			reading = stk3x3x_get_als_reading(stk_data);
#ifdef STK_ALS_FIR
			memset(&stk_data->fir, 0x00, sizeof(stk_data->fir));
		...
		if(stk_data->prox_debug == false) {
			...
			if (near_far_state == 0) {
				if(reading > (stk_data->psi + STK_H_PS)) {
					...
					stk3x3x_set_ps_thd_l(stk_data, stk_data->ps_thd_l);
					stk_data->ps_thd_update = true;
					SENSOR_LOG_INFO(" near update HT=%d, LT=%d\n", stk_data->ps_thd_h, stk_data->ps_thd_l);
				}
			} else {
				SENSOR_LOG_INFO(" far ps_update=%d\n", stk_data->ps_thd_update);
...
static void stk3x3x_als_set_poll_delay(struct stk3x3x_data *stk_data)
{
	...
	if(als_it == STK_IT_ALS_25MS) {
		...
		stk_data->als_poll_delay = ns_to_ktime(110 * NSEC_PER_MSEC);
		SENSOR_LOG_INFO("unknown ALS_IT=%d, set als_poll_delay=110ms\n", als_it);
	}
...
static int stk3x3x_set_als_ctrl(struct stk3x3x_data *stk_data, uint8_t als_it, uint8_t als_gain)
{
	...
	if(als_it == STK_IT_ALS_100MS && als_gain == STK_ALS_GAIN_64) {
		stk_data->als_dim_gain = true;
		stk_data->als_fast_report = false;
		stk_data->als_fast_report_cnt = 0;
	} else if(als_it == STK_IT_ALS_25MS && als_gain == STK_ALS_GAIN_16) {
		stk_data->als_fast_report = true;
		stk_data->als_fast_report_cnt = 20;
		stk_data->als_dim_gain = false;
	} else {
		stk_data->als_dim_gain = false;
		stk_data->als_fast_report = false;
		stk_data->als_fast_report_cnt = 0;
...
static int stk3x3x_set_input_devices(struct stk3x3x_data *stk_data)
{
	...
	set_bit(REL_Y,  stk_data->als_input_dev->relbit);
