...
extern void sec_nfc_set_mode(struct sec_nfc_info *info, enum sec_nfc_mode mode);
...
detect_end:
	...
	if(ret){
		...
		printk(KERN_DEBUG "%s:SEC NFC chip detect Ok!\n", __func__);
	}
...
void sec_nfc_set_mode(struct sec_nfc_info *info,
                    enum sec_nfc_mode mode)
{
    struct sec_nfc_platform_data *pdata = info->pdata;
...
static int sec_nfc_suspend(struct device *dev)
{
    ...
    if(!info){
        pr_info("%s: info : sec nfc driver have destroyed!\n", __func__);
        return 0;
    }
