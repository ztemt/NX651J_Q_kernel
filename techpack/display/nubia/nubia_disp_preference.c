		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	if(nubia_display == NULL || !nubia_disp_val.en_dfps) {
		NUBIA_DISP_ERROR("nubia_display is NULL or no dpfs\n");
		return size;
	}
	...
	if ((val == DFPS_144) && !nubia_display->panel->supported_144hz) {
		NUBIA_DISP_ERROR("the panel not supported 144HZ \n");
		return size;
	}

	NUBIA_DISP_INFO("dfps value = %d\n", val);
...
bool rtn_check(struct dsi_panel *panel, int32_t rtn)
{
	if (!panel) {
		NUBIA_DISP_ERROR("Invalid Params\n");
		return false;
	}

	switch (panel->cur_fps) {
		case FPS_60:
			if ((rtn > 0x5C) || (rtn < 0x59))
				return false;
			else
				return true;
			break;
		case FPS_90:
			if ((rtn > 0x3D) || (rtn < 0x3B))
				return false;
			else
				return true;
			break;
		case FPS_120:
			if ((rtn > 0x3B) || (rtn < 0x39))
				return false;
			else
				return true;
			break;
		case FPS_144:
			if ((rtn > 0x32) || (rtn < 0x30))
				return false;
			else
				return true;
			break;
		default:
			NUBIA_DISP_ERROR("unsupported frame rate!! \n");
			break;
	}
		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%x", &val);
	NUBIA_DISP_INFO("fps change write value = 0x%x",val);

	panel = nubia_display->panel;
	if (!rtn_check(panel, val)) {
		nubia_disp_val.fps_change = val;
		NUBIA_DISP_INFO("fps_change out of range! cur_fps = %d, fps_change = 0x%x\n", panel->cur_fps, val);
		return size;
	}

	ret = nubia_dsi_panel_dfps_change(panel, val);
	if (ret == 0) {
		nubia_disp_val.fps_change = val;
		NUBIA_DISP_INFO("success to set dfps as = 0x%x\n", val);
	}
